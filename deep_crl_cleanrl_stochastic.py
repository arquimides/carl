...
from experiments_configurations.config import EvaluationMetric, EpisodeStateInitialization, Times, \
    ...
    EnvironmentType, CARLDQNConf, DQNConf, CARLC51Conf
...
class C51DQN:
    def __init__(self, env, num_envs, screen_width, screen_height, n_atoms, v_min, v_max, learning_rate,
                 buffer_size, gamma, target_network_update_frequency, batch_size, start_e, end_e, exploration_fraction,
        ...
        self.q_network = QNetwork(self.envs, n_atoms=self.n_atoms, v_min=self.v_min, v_max=self.v_max).to(self.device)
        self.optimizer = optim.Adam(self.q_network.parameters(), lr=self.learning_rate, eps=0.01 / self.batch_size)
        self.target_network = QNetwork(self.envs, n_atoms=self.n_atoms, v_min=self.v_min, v_max=self.v_max).to(self.device)
                    ...
                    if action_indexes:
                        if crl_action_selection_strategy == ActionSelectionStrategy.MODEL_BASED_EPSILON_GREEDY:
                            ...
                            if random.random() < epsilon_values[initial_epsilon_index + episode]:  # Explore
                                ...
                                actions, pmf = self.q_network.get_action(torch.Tensor(obs).to(self.device))
                ...
                if self.global_step > self.learning_start:
                    if self.global_step % self.train_frequency == 0:
                        ...
                        with torch.no_grad():
                            ...
                            next_atoms = data.rewards + self.gamma * self.target_network.atoms * (1 - data.dones)
                            # projection
                        ...
                        if self.global_step % 100 == 0:
                            writer.add_scalar("losses/loss", loss.item(), self.global_step)
...
if __name__ == '__main__':
    ...
    # Single test
    experiments_to_run = config.exp_deep_rl_2
    ...
    for experiment_conf in experiments_to_run:
        ...
        for t in range(trials):
            ...
            for a in range(number_of_algorithms):
                ...
                #  Algorithm
                n_atoms = algorithm.n_atoms
                v_min = algorithm.v_min
                ...
                if isinstance(algorithm, CARLDQNConf) or isinstance(algorithm, CARLC51Conf):
                ...
                agent = C51DQN(env, 1, screen_width, screen_height, n_atoms, v_min, v_max,
                                learning_rate, buffer_size, gamma, target_network_update_frequency, batch_size, start_e, end_e, exploration_fraction,
                                learning_start, train_frequency)
